...
const WeatherWidget: React.FC = () => {
  ...
  const fetchWeather = async (q: string) => {
    try {
      const res = await axios.get(
        `http://localhost:4000/api/weather?city=${q}`
      );
  ...
  const getWeatherIcon = (desc: string) => {
    ...
    if (desc.includes("storm"))
      return <ThunderstormIcon style={{ fontSize: 40 }} />;
            ...
            <>
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  {report?.main.humidity || currreport?.main.humidity}%
                </Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <WindPowerIcon /> Wind:{" "}
                  {report?.wind.speed || currreport?.wind.speed} kph
                </Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <CompressIcon /> Pressure:{" "}
                  {report?.main.pressure || currreport?.main.pressure} mb
                </Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <DeviceThermostatIcon /> Feels Like:{" "}
                  {Math.round(
                    (report?.main.feels_like || currreport?.main.humidity) ?? 0
                  )}
                  °C
                </Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  💧 Precipitation:{" "}
                  {report?.main.sea_level || currreport?.main.sea_level} in
                </Box>
              </Box>
